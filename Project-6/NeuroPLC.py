...
class anIndividual:
    ...
    def randomly_generate(self, lb, ub):
        for i in range(self.chromosome_length):
            self.chromosome.append(random.choice([0, random.randint(lb, ub)]))
...
class aSimpleExploratoryAttacker:
    ...
    def evolutionary_cycle(self):
        ...
        if self.strategy == "ElitistGen-GA":
            ...
            raise ValueError('Make sure the strategy is selected correctly')
...
def NeuralNetwork(LayerList):
    ...
    # Load Data
    df = pd.ExcelFile('All.xlsx')
    X_train = np.array(df.parse('All'))
    y_train = np.array(df.parse('Output'))
    ...
    for i in range(len(LayerList)):
        if int(LayerList[i]) and int(LayerList[i]) > 7:
            NeuralNetwork.add(tf.keras.layers.Dense(int(LayerList[i]), activation=tf.nn.relu))
            LayerCount += 1
    NeuralNetwork.add(tf.keras.layers.Dense(7, input_shape=(7,)))
    print("Layers added: " + str(LayerCount))

    # Train Model
    NeuralNetwork.compile(optimizer='sgd', loss='mean_squared_error', metrics=['accuracy', 'binary_accuracy'])
    ...
    val_loss, val_acc, val_bin_acc = NeuralNetwork.evaluate(X_train, y_train)
    print("Test loss:" + str(val_loss) + "   " + "Test acc:" + str((val_acc + val_bin_acc) / 2))
    return (val_acc+val_bin_acc)/2
...
for run in range(runCount):
    ...
    for j in range(len(chromosome)):
        if chromosome[j] and chromosome[j]>7:
...
print(df)
df.to_csv('Results_NeuroPLC.csv')